...
//	$scope.checkSelectedYb = function(searchText) {
	...
	$scope.insertEcuVarVerToGlobal = function() {
		...
		angular.forEach($scope.var_ver_selected_new_entry, function(ecu_var_ver) {
			...
			//if(xmit == null || gwbitpos == null || gwbitpos == "" || bus == null) {  -> For gateway ecus xmit and gateway bit pos is null
			if(bus === null) { 
...
//		$q.all(promises)
//			.then(function() {
//				$scope.varver_insert_status.value = "Global DB update succeed";
//				$scope.showEngVehConfigs($scope.selectedYb);
//			}, function() {
//				$scope.varver_insert_status.value = "Global DB update failed";
//			});
	};

	function insertEcuVarVerToFlobalSuccess(response) {
		$scope.showEngVehConfigs($scope.selectedYb);
		console.log("Global DB update succeed", response);
		$scope.varver_insert_status.value = "Global DB update succeed";
	}

	function insertEcuVarVerToFlobalFailed(response) {
		console.log("Global DB update failed", response);
		if(response)
			$scope.varver_insert_status.value = "Global DB update failed: " + response.code + "|" + response.message;
		else
			$scope.varver_insert_status.value = "Global DB update failed";
	}
...
//	$scope.set_selected_checkbox_new_entry = function(ecu_var_ver_clicked) {
//		ecu_var_ver_clicked.selected = !ecu_var_ver_clicked.selected
//		angular.forEach($scope.var_ver_selected_new_entry, function(ecu_var_ver) {
//			if(ecu_var_ver_clicked != ecu_var_ver) {
//				ecu_var_ver.selected = false;
